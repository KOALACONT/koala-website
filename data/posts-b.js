/* Guides for Koala Containers, part two. Split from posts.js on 14/09/2026
   purely to keep each file under 100KB for the tooling that pushes them;
   posts.js concatenates this array onto its own. Same shape per entry. */
module.exports = [
  {
...
<p>Loading the top unit is its own problem. Everything has to go up. In practice a stacked upper container works well for things that go in once and come out once — seasonal stock, archives, spares — and badly for anything you need weekly. If you plan to use the top unit regularly you need a way up to it: a platform, a landing, a properly rated forklift work platform where your site's safety rules allow it, or stairs. That is a structure, and structures people climb need designing.</p>
